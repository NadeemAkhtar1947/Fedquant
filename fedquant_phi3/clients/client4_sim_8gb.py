...
print("CLIENT 4 — Simulated 8GB Phone")

# ── Step 1: Load Data ──────────────────────────────
print("\n[Step 1] Loading data...")
with open("./data/clients/client4.json", "r", encoding="utf-8") as f:
    data = json.load(f)
data = data
...
print("  Note: Client 4 uses 8GB RAM limit (medium tier phone)")
...
print(f"  RAM limit: 8GB (simulating medium tier phone)")

# ── Step 3: Attach LoRA Adapter ───────────────────
print("\n[Step 3] Attaching LoRA adapter...")

lora_config = LoraConfig(
    r=16,                         # ← r=16 instead of r=8 (more capacity for 8GB tier)
    lora_alpha=32,
...
print("  Saved to ./results/adapters/client4")

print("CLIENT 4 DONE — Adapter ready to send to server")